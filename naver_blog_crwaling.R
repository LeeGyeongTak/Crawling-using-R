...
blog_crawl<-function(keyword,number_of_blog){
  nc<-number_of_blog
  keyword<-iconv(keyword,from="CP949",to="UTF-8")
  Encoding(keyword)
  keyword2<-URLencode(keyword)
  
  # https://search.naver.com/search.naver?where=post&query=%EB%94%A5%EB%9F%AC%EB%8B%9D&ie=utf8&st=sim&sm=tab_opt&date_from=20170401&date_to=20170504&date_option=8&srchby=all&dup_remove=1&from20170401to20170504&mson=0
